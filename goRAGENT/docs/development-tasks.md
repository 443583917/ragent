...
> 决策：`t_intent_node` 表重建对齐 Java DDL（数值 level/kind、独立 id 主键、补 sort_order）；
> Go 版暂无查询改写 → 整问题作为单一子问题分类（一次 LLM 调用，免线程池）；MCP 意图仅流转不执行（P2）。

| # | 任务 | 对应原任务 | 产出文件 |
|:--|------|:--------:|---------|
| I-1 | 重建 t_intent_node 表（对齐 Java DDL + 前端契约） | 1.4 | `docker/init.sql` |
...
| I-7 | 管理后台 intent-tree CRUD 实装（7 端点对齐 Java 契约，写后清缓存） | 4.10 | `internal/admin/intent_tree.go` |
...
## 迁移路线图（2026-07-17 基于 Java 版全量功能盘点重排）

> 依据：Java 版 8 阶段 Pipeline / 记忆 / MCP / 检索 / 限流 / 管理后台全量对比。
> 原则：先修"文档虚标但实际是空壳"的核心体验（记忆/会话），再补检索质量，再扩平台能力。
...
| # | 任务 | 对照 Java | 产出 |
|:--|------|----------|------|
| M1-1 | 记忆加载实装：按 conversationId 取 historyKeepTurns×2 条，去前导 assistant，保留 user/assistant 对 | JdbcConversationMemoryStore.loadHistory | `internal/rag/memory/memory.go` + 测试 |
| M1-2 | 消息持久化：user 消息进 Pipeline 时落库；assistant 消息 OnComplete 落库 | JdbcConversationMemoryStore.append + StreamChatEventHandler | `memory.go`, `pipeline/handler.go` |
| M1-3 | 会话 createOrUpdate：首条消息建 t_conversation（标题=问题截断，LLM 标题 M3 再做） | ConversationService | `memory.go` |
| M1-4 | 会话管理 API 实装：列表/重命名/删除/消息历史 | ConversationController | `internal/rag/session.go` + 测试 |
| M1-5 | 消息反馈实装（点赞/踩，同步写库即可，不引 MQ） | MessageFeedbackController | `session.go` |
...
| # | 任务 | 对照 Java | 产出 |
|:--|------|----------|------|
| M2-1 | 关键词映射 CRUD 实装（t_query_term_mapping） | QueryTermMappingController | `internal/admin/` |
| M2-2 | 同义词归一化 | QueryTermMappingService.normalize | `internal/rag/rewrite/term_mapping.go` |
| M2-3 | LLM 查询改写 + 子问题拆分（JSON 输出，失败回退规则拆分按 ？?。；; ） | MultiQuestionRewriteService | `internal/rag/rewrite/` |
| M2-4 | 意图解析升级：按子问题并行分类 + capTotalIntents 保底分配 | IntentResolver.capTotalIntents | `internal/rag/intent/resolver.go` |
| M2-5 | 空检索短路：未命中文档直接返回提示语，不走 LLM | handleEmptyRetrieval | `internal/rag/pipeline/pipeline.go` |
| M2-6 | Prompt 场景规划（KB_ONLY/EMPTY + 意图节点自带模板覆盖） | RAGPromptService.plan | `internal/rag/prompt/planner.go` |

### M3：摘要压缩 + 歧义引导 + SYSTEM 短路（P2 · 对话增强）✅ 已完成并验收（2026-07-17）

| # | 任务 | 对照 Java | 产出 |
|:--|------|----------|------|
| M3-1 | 摘要压缩：用户消息数≥summaryStartTurns 异步触发，Redis SETNX 锁，LLM 压缩落 t_conversation_summary | JdbcConversationMemorySummaryService | `internal/rag/memory/summary.go` |
| M3-2 | 摘要拼接进历史（system 角色 summary-wrapper 包裹置顶） | attachSummary | `memory.go` |
| M3-3 | LLM 会话标题生成 | ConversationTitleGenerator | `memory.go` |
...
| # | 任务 | 对照 Java | 产出 |
|:--|------|----------|------|
| M4-1 | 知识库 CRUD 实装 | KnowledgeBaseController | `internal/admin/` |
| M4-2 | 文档上传/列表/详情/启停用/删除 | DocumentController | `internal/admin/` + 文件存储 |
| M4-3 | 入库 Pipeline：解析（MinerU 接线）→分块→嵌入→Milvus 索引 | core/chunk + core/parser + Indexer | `internal/ingestion/` |
| M4-4 | Chunk 管理 CRUD + 启停用 | ChunkController | `internal/admin/` |
...
| # | 任务 | 对照 Java | 产出 |
|:--|------|----------|------|
| M5-1 | MCP 客户端 + 工具注册表（连接远程 MCP Server, tools/list 发现） | McpClientAutoConfiguration | `internal/rag/mcp/` |
| M5-2 | LLM 参数提取（支持节点 param_prompt_template） | LLMMcpParameterExtractor | `internal/rag/mcp/extractor.go` |
| M5-3 | 工具执行 + 结果格式化进上下文 | RetrievalEngine.executeMcpTools | `internal/rag/mcp/` |
...
| # | 任务 | 对照 Java | 产出 |
|:--|------|----------|------|
| M6-1 | 公平分布式限流（Lua 原子出队 + pub/sub + SSE REJECT 事件） | FairDistributedRateLimiter | `internal/framework/ratelimit/` |
| M6-2 | 停止任务实装 + 幂等控制 | StopTask + @IdempotentSubmit | `pipeline/handler.go` |
| M6-3 | Trace 落库（节点耗时/TTFT → t_rag_trace_run/node）+ 查询 API 实装 | RagTraceAspect | `internal/framework/trace/` |
| M6-4 | 审计日志（变更快照落 t_biz_change_log + 查询 API） | BizChangeLogRecordService | `internal/admin/` |
| M6-5 | 仪表板真实统计（overview/performance/trends） | DashboardController | `internal/admin/` |
| M6-6 | 用户管理 CRUD + 改密、示例问题 CRUD 实装 | UserController 等 | `internal/admin/`, `internal/user/` |
| M6-7 | （可选）ES 关键词检索通道 | KeywordSearchChannel | `internal/rag/retrieve/` |
...
**目标**：项目可编译启动，可连接所有外部依赖，SSE 事件格式和 Java 版一致。
...
| 5.10 | 文档同步检查 | P1 | 2h | 所有接口和 Java 版对比 |
...
| Go 版和 Java 版行为不一致 | Phase 2 回答质量 | 逐阶段 A/B 对比测试 |
