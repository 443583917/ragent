...
func (h *Handler) listDocuments(c *gin.Context) {
	...
	var total int64
	h.db.WithContext(c.Request.Context()).Model(&model.DocumentDO{}).
...
func (h *Handler) searchDocuments(c *gin.Context) {
	...
	var dos []model.DocumentDO
	var total int64
	query := h.db.WithContext(c.Request.Context()).Model(&model.DocumentDO{}).Where("deleted = 0")
	if keyword != "" {
...
func (h *Handler) deleteDocument(c *gin.Context) {
	...
	h.db.WithContext(c.Request.Context()).Model(&model.DocumentDO{}).Where("id = ?", id).Update("deleted", 1)
	h.db.WithContext(c.Request.Context()).Model(&model.ChunkDO{}).Where("doc_id = ?", id).Update("deleted", 1)
