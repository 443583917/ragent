...
func buildIntentTreeVOs(dos []model.IntentNodeDO) []*IntentNodeTreeVO {
...
func createReqToDO(req intentNodeCreateReq, id, operator string) model.IntentNodeDO {
...
func (h *Handler) createIntentNode(c *gin.Context) {
	...
	do := createReqToDO(req, snowflake.NextID(), middleware.GetUserID(c.Request.Context()))
...
func (h *Handler) updateIntentNode(c *gin.Context) {
	...
	updates := updateReqToUpdates(req, middleware.GetUserID(c.Request.Context()))
	if err := h.db.WithContext(c.Request.Context()).
