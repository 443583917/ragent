...
func (h *Handler) dashboardStatsReal(c *gin.Context) {
	...
	h.db.WithContext(c.Request.Context()).Model(&model.KnowledgeBaseDO{}).Where("deleted = 0").Count(&kb)
	h.db.WithContext(c.Request.Context()).Model(&model.DocumentDO{}).Where("deleted = 0").Count(&docs)
...
func (h *Handler) dashboardPerformanceReal(c *gin.Context) {
	...
	h.db.WithContext(c.Request.Context()).Model(&model.TraceRunDO{}).Count(&total)
	h.db.WithContext(c.Request.Context()).Model(&model.TraceRunDO{}).Where("status = 'EMPTY' AND error_message = ''").Count(&noDoc)
	h.db.WithContext(c.Request.Context()).Model(&model.TraceRunDO{}).Where("error_message != ''").Count(&errCount)
...
func (h *Handler) dashboardTrendsReal(c *gin.Context) {
	...
	if metric == "quality" {
		...
		for i := days - 1; i >= 0; i-- {
			...
			var runs int64
			h.db.WithContext(c.Request.Context()).Model(&model.TraceRunDO{}).
