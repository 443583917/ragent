...
type Classifier struct {
	...
	treeOverride []*model.IntentNode // 测试注入用
}
...
func (c *Classifier) ClassifyTargets(ctx context.Context, question string) []NodeScore {
	...
	id2node := make(map[string]*model.IntentNode, len(leaves))
...
func buildIntentList(leaves []*model.IntentNode) string {
	...
	for i, n := range leaves {
		...
		b.WriteString("\n  type=" + n.Kind.String())
		if n.Kind == model.KindMCP && n.McpToolID != "" {
			b.WriteString("\n  toolId=" + n.McpToolID)
		}
		if len(n.Examples) > 0 {
...
func parseResponse(raw string, id2node map[string]*model.IntentNode) []NodeScore {
