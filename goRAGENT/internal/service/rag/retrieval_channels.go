...
type IntentDirectedSearchChannel struct {
	...
	retriever model.VectorRetriever
}

func NewIntentDirectedChannel(cfg config.IntentDirectedConfig, retriever model.VectorRetriever) *IntentDirectedSearchChannel {
...
func (c *IntentDirectedSearchChannel) Type() model.SearchChannelType { return model.ChannelIntentDirected }

func (c *IntentDirectedSearchChannel) IsEnabled(ctx context.Context, sc *model.SearchContext) bool {
	if !c.cfg.Enabled {
		return false
	}
...
func (c *IntentDirectedSearchChannel) Search(ctx context.Context, sc *model.SearchContext) (*model.ChannelResult, error) {
	start := time.Now()
	kbIntents := sc.KBIntents()

	if len(kbIntents) == 0 {
		return emptyResult(model.ChannelIntentDirected, c.Name(), start), nil
	}
	...
	return &model.ChannelResult{
		ChannelType: model.ChannelIntentDirected,
		ChannelName: c.Name(),
		Chunks:      allChunks,
		LatencyMs:   latency,
	}, nil
...
func NewVectorGlobalChannel(cfg config.VectorGlobalConfig, intentDirectedEnabled bool, retriever model.VectorRetriever) *VectorGlobalSearchChannel {
...
func (c *VectorGlobalSearchChannel) Type() model.SearchChannelType   { return model.ChannelVectorGlobal }

func (c *VectorGlobalSearchChannel) IsEnabled(ctx context.Context, sc *model.SearchContext) bool {
	// 1. 配置关闭
	if !c.cfg.Enabled {
		return false
	}
...
func (c *VectorGlobalSearchChannel) Search(ctx context.Context, sc *model.SearchContext) (*model.ChannelResult, error) {
	start := time.Now()

	collections, err := c.retriever.ListCollections(ctx)
	if err != nil || len(collections) == 0 {
		zap.L().Warn("未找到任何 KB collection，跳过全局检索")
		return emptyResult(model.ChannelVectorGlobal, c.Name(), start), nil
	}
	...
	return &model.ChannelResult{
		ChannelType: model.ChannelVectorGlobal,
		ChannelName: c.Name(),
		Chunks:      allChunks,
		LatencyMs:   latency,
	}, nil
...
func emptyResult(ct model.SearchChannelType, name string, start time.Time) *model.ChannelResult {
	return &model.ChannelResult{
		ChannelType: ct,
		ChannelName: name,
		Chunks:      nil,
...
func (c *YouComWebSearchChannel) Type() model.SearchChannelType { return model.ChannelWebSearch }

func (c *YouComWebSearchChannel) IsEnabled(ctx context.Context, sc *model.SearchContext) bool {
	return c.cfg.Enabled && c.cfg.APIKey != ""
}

func (c *YouComWebSearchChannel) Search(ctx context.Context, sc *model.SearchContext) (*model.ChannelResult, error) {
	start := time.Now()
	...
	if err != nil {
		return emptyResult(model.ChannelWebSearch, c.Name(), start), nil
	}
	...
	return &model.ChannelResult{
		ChannelType: model.ChannelWebSearch,
		ChannelName: c.Name(),
		Chunks:      chunks,
		LatencyMs:   latency,
	}, nil
