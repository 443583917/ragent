...
func TestConvToVO_Format(t *testing.T) {
	...
	vo := convToVO(model.ConversationDO{
...
func TestConvToVO_ZeroTimeEmpty(t *testing.T) {
	vo := convToVO(model.ConversationDO{ConversationID: "c1"})
	if vo.LastTime != "" {
...
func TestMsgToVO_AllFields(t *testing.T) {
	...
	vo := msgToVO(model.ConversationMessageDO{
...
func TestMsgToVO_NullableFields(t *testing.T) {
	vo := msgToVO(model.ConversationMessageDO{ID: 1, Role: "user", Content: "问"})
