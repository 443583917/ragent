...
func (d *Detector) Detect(ctx context.Context, question string, subs []model.SubQuestionIntent) string {
...
func (d *Detector) confirmByLLM(ctx context.Context, question string, ranked []model.NodeScore) bool {
...
func (d *Detector) buildGuidancePrompt(ranked []model.NodeScore) string {
...
func rankCandidates(subs []model.SubQuestionIntent) []model.NodeScore {
	...
	ranked := make([]model.NodeScore, 0, len(best))
	for _, ns := range best {
...
func renderOptions(ranked []model.NodeScore, max int) string {
	var b strings.Builder
...
func domainMentioned(question string, ranked []model.NodeScore) bool {
	for _, ns := range ranked {
