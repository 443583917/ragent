...
type IntentResolver interface {
	ResolveAll(ctx context.Context, subQuestions []string) []model.SubQuestionIntent
}

// QueryRewriter 查询改写抽象（*model.Rewriter 满足；nil 表示未启用）
type QueryRewriter interface {
	RewriteWithSplit(ctx context.Context, question string, history []llm.Message) RewriteResult
}

// GuidanceDetector 歧义引导抽象（*guidance.Detector 满足；nil 表示未启用）
// 返回引导话术，空串表示无需引导
type GuidanceDetector interface {
	Detect(ctx context.Context, question string, subs []model.SubQuestionIntent) string
}

// MemoryService 对话记忆抽象（*memory.ConversationMemory 满足）
type MemoryService interface {
	LoadAndAppend(ctx context.Context, conversationID, userID string, msg model.ChatMessage) []model.ChatMessage
...
func NewSimplePipeline(cfg *config.Config, mem MemoryService, llmSvc *llm.ChatService,
	prompts *prompt.TemplateLoader, re *RetrievalEngine,
