...
func buildSummaryMessages(sysPrompt string, msgs []ConversationMessageDO, existing string, maxChars int) []llm.Message {
...
func (m *ConversationMemory) maybeCompress(conversationID, userID string) {
	...
	// 用户消息总数达到触发轮数
	var total int64
	if err := m.db.WithContext(ctx).Model(&ConversationMessageDO{}).
	...
	var window []ConversationMessageDO
	...
	var toSummarize []ConversationMessageDO
	if err := m.db.WithContext(ctx).
