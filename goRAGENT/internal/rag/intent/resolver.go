...
func (r *Resolver) Resolve(ctx context.Context, question string) []retrieve.SubQuestionIntent {
...
func (r *Resolver) ResolveAll(ctx context.Context, subQuestions []string) []retrieve.SubQuestionIntent {
	...
	subs := make([]retrieve.SubQuestionIntent, 0, len(subQuestions))
	for i, q := range subQuestions {
		scores := results[i]
		if len(scores) == 0 {
			continue
		}
		nodeScores := make([]retrieve.NodeScore, len(scores))
		for j, ns := range scores {
			nodeScores[j] = retrieve.NodeScore{Node: toNodeRef(ns.Node), Score: ns.Score}
		}
		subs = append(subs, retrieve.SubQuestionIntent{SubQuestion: q, NodeScores: nodeScores})
	}
...
func capTotalIntents(subs []retrieve.SubQuestionIntent, max int) []retrieve.SubQuestionIntent {
	total := 0
	for _, s := range subs {
	...
	grouped := make(map[int][]retrieve.NodeScore, len(subs))
	for i, c := range cands {
		if picked[i] {
			grouped[c.subIdx] = append(grouped[c.subIdx], c.ns)
		}
	}
	var result []retrieve.SubQuestionIntent
	for i, s := range subs {
		if scores := grouped[i]; len(scores) > 0 {
			result = append(result, retrieve.SubQuestionIntent{SubQuestion: s.SubQuestion, NodeScores: scores})
		}
	}
	return result
}
...
// toNodeRef IntentNode → retrieve.NodeRef（检索层轻量引用）
func toNodeRef(n *IntentNode) *retrieve.NodeRef {
	return &retrieve.NodeRef{
		ID:             n.ID,
		Name:           n.Name,
		FullPath:       n.FullPath,
